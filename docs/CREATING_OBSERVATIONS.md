...
The creation wizard guides you through 7 steps: **Project**, **Target & Exposure**, **Constraints**, **Repetition**, **Autofocus**, **Guiding**, and **Review & Submit**.

### Step 1: Project

1. Provide a descriptive name for the Observation being created
...
4. Optionally select an external storage option where FITS files generated by the Observation will be copied to by the server
...
Priority is set per-target within the Target & Exposure step using a numeric **1–10** scale (default **5**, where **10** is the highest priority):

| Priority | Use For |
|----------|---------|
| **8–10** | Time-critical, urgent (transits, occultations, ToO) |
| **5–7** | Standard science (5 is the default) |
| **1–4** | Fill time, surveys |

When multiple observations are eligible, the scheduler runs the one with the highest priority number first. See [Scheduler Features](SCHEDULER_FEATURES.md#priority-system) for details.
...
Configure how guiding behaves during this observation:

- **Use observatory defaults** (recommended) — inherits the guiding and dithering configuration set by the observatory administrator
- **Custom settings** — enable or disable guiding and dithering for this specific observation

For detailed information on guiding options, see the **[Guiding Guide](GUIDING_GUIDE.md)**.
...
- Constraint violations are shown on the **observation list** via the DispatchStatusIndicator tooltip on pending/waiting observations
...
2. Click the **Edit** icon button on the observation row, or right-click for the context menu and select **Edit** (also available from the Observation Timeline view)
3. This navigates to the edit page for the observation
...
1. Find the observation in the **observation list** or **project view**
2. Click **Resubmit**
...
Any active observation can also end in Failed, Aborted, or Cancelled,
or be Paused / Partially Completed depending on the situation.
```
...
| **Paused** | A repetitive series has been paused; no new executions scheduled |
| **Partially Completed** | Ended with some, but not all, planned exposures captured |
...
2. Click on the observation to open details, where files are displayed inline
3. Download individual files or use batch download
...
- Use **5** (the default) for most observations
- Reserve **8–10** for truly time-critical work
- Use **1–4** for fill-time programs that can run anytime
